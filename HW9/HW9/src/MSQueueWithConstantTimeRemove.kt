...
class MSQueueWithConstantTimeRemove<E> : QueueWithRemove<E> {
    ...
    private class Node<E>(
        ...
        fun remove(): Boolean {
            ...
            val nextElement = next.get() ?: return isRemoved
            val prevElement = prev.get() ?: return isRemoved
            prevElement.next.set(nextElement)
            while (true) {
                val currentPrevNext = nextElement.prev.get() ?: break
                if (nextElement.prev.compareAndSet(currentPrevNext, prevElement)) break
            }
            if (prevElement.extractedOrRemoved) {
                prevElement.remove()
            }
            if (nextElement.extractedOrRemoved) {
                nextElement.remove()
            }
            return isRemoved
        }
